...
public class ThreeSum {
    ...
    public List<List<Integer>> threeSumV2(int[] nums) {
        ...
        for (int i = 0; i < sortedList.size() - 2; i++) {
            if (i == 0 || (i > 0 && sortedList.get(i) != sortedList.get(i - 1))) {
                int left = i + 1;
                int right = sortedList.size() - 1;
                int needSum = -sortedList.get(i); // 需要找到两个数组合结果是needSum的bu
                while (left < right) {
                    int tmp = sortedList.get(left) + sortedList.get(right);
                    if (needSum == tmp) {
                        list.add(Arrays.asList(sortedList.get(left), sortedList.get(right), sortedList.get(i)));
                        // 重复数据不在比价了
                        while (left < right && sortedList.get(left) == sortedList.get(left + 1)) left++;
                        while (left < right && sortedList.get(right) == sortedList.get(right - 1)) right--;
                        left++;
                        right--;
                    } else if (tmp < needSum) {
                        left++;
    ...
    /**
     * leetcode 上的讨论版本 时间复杂度n2
     *
     * @param num
     * @return
     */
    public List<List<Integer>> threeSumV3(int[] num) {
        Arrays.sort(num);
